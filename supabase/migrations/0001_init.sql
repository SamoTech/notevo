...
-- Tags table (separate entity for better management - Laverna feature)
...
-- User settings table (Laverna configs feature)
create table if not exists public.user_settings (
  ...
  keybindings_preset      text        not null default 'default',
...
create index if not exists tags_name_idx          on public.tags      (user_id, name);
...
drop policy if exists "delete_own_settings" on public.user_settings;
...
create policy "insert_own_tags" on public.tags
  for insert with check (auth.uid() = user_id);
create policy "update_own_tags" on public.tags
  for update using (auth.uid() = user_id);
...
create policy "update_own_settings" on public.user_settings
  for update using (auth.uid() = user_id);
create policy "delete_own_settings" on public.user_settings
  for delete using (auth.uid() = user_id);
